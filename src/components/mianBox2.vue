                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      ...
                      <h4 class="modal-title">添加领导入户慰问</h4>
                    </div>
                    ...
                    <div class="modal-body  ">
                      ...
                      <table class="footable table table-stripped toggle-arrow-tiny">
                        ...
                        <tbody>
                        <tr v-for="(item,index ) in leaderList"
                            :key="item.id">
                          <td><input type="checkbox" v-model="checkboxidList" :value="item.id"></td>
                          <td>{{item.name}}</td>
                          <td>{{item.number}}</td>
                        ...
                        <tr>
                          <td colspan="4">
                            ...
                            <zpagenav v-bind:page="page2" v-bind:page-size="pageSize2" v-bind:total="total2"
                                      v-bind:max-page="maxPage2" v-on:pagehandler="findAllLeader">
                            </zpagenav>
                          </td>
                        </tr>
                        </tfoot>
                ...
                <div class="modal-dialog">
                  <div class="modal-content animated bounceInRight">
                    <div class="modal-header">
                      ...
                      <h4 class="modal-title"><a>查看领导入户慰问信息</a></h4>
                    </div>
                    <div class="modal-body form-inline ">
                      <table class="footable table table-stripped toggle-arrow-tiny">
                        ...
                        <tr v-for="(item,index ) in list1"
                          ...
                          <td>{{item.coodepart}}</td>
                          <td>{{item.contacttel}}</td>
                          <td v-if="showdelete=='show'"><a class="btn btn-danger btn-xs" style="color: white"
                                                     @click="deleader(item.id)">删除</a></td>
                        </tr>
                        </tbody>
                        <tfoot>
                        <tr>
                          <td colspan="4">
                            ...
                            <zpagenav v-bind:page="page1" v-bind:page-size="pageSize1" v-bind:total="total1"
                                      v-bind:max-page="maxPage1" v-on:pagehandler="detail">
                            </zpagenav>
                          </td>
                        </tr>
                        </tfoot>
              ...
              <form role="form" class="form-inline">
                <div class="form-group">
                  <a class="btn btn-primary" style="color: white"
                     @click="pageHandler(1,selectname,'tongguotijiao')">全部</a>&nbsp;&nbsp;&nbsp;
                ...
                <tr v-for="item in list" :key="item.id">
                  ...
                  <td style="width: 400px;">
                    <a class="btn btn-primary btn-xs" style="color: white" data-toggle="modal"
                       data-target="#myModal3" @click="detail(1,item.id)">查看领导入户慰问信息</a>
                    <!--v-if="item.statusname==='退回'"-->
                    <a v-if="item.statusname==='通过'" class="btn btn-primary btn-xs" data-toggle="modal"
                       data-target="#myModal4" style="color: white" @click="findAllLeader(1,item.id)">添加领导入户慰问</a>
                    <a v-if="item.statusname==='通过'" class="btn btn-primary btn-xs" style="color: white"
                       @click="submit(item.id)">完成</a>
                  </td>
                </tr>
                </tbody>
                <tfoot>
...
<script>
  ...
  export default {
    ...
    created() {
      this.pageHandler(1, null, 'tongguotijiao');
    ...
    watch: {
      'selectname': function name() {
        ...
        this.pageHandler(1, this.selectname, "tongguotijiao");
      }
    },
    methods: {
      deleader(id) {

        this.$axios.get('/condolence/deleteleader', {
          params: {id: id}
        })
          .then((response) => {
            this.detail(this.page1, this.temp1);
      ...
      IsShow(id){
        this.$axios.get('/condolence/info4', {
          params: {
            id: id
          }
        })
      ...
      detail(page, id) {
       this.IsShow(id);
        ...
        var that = this;
          that.$axios.get('/condolence/info3', {
            params: {
              id: that.temp1,
              page: that.page1,
            ...
            .then((response) => {
              that.list1 = response.data.list;

              that.total1 = response.data.total;
              that.maxPage1 = response.data.pages;
              that.pageSize1 = response.data.pageSize;
      ...
      ,
      findAllLeader(page, id) {
        this.page2 = page;
        if (!this.isEmpty(id)) {
          this.temp = id;
        }
        this.$axios.get('/condolence/findAllLeader', {
          params: {
            difficultpartyid: this.temp,
            page: this.page2,
          ...
          .then((response) => {
            ...
            this.leaderList = response.data.list;
            this.total2 = response.data.total;
            this.maxPage2 = response.data.pages;
      ...
      insertLeader(id) {
        if (this.checkboxidList.length == 0) {
          ...
          this.text = "modal";
          this.$axios.get('/condolence/insertLeader', {
            params: {
              difficultpartyid: this.temp,
              checkboxidList: JSON.stringify(this.checkboxidList)
            }
          })
            .then((response) => {
              console.log(response);
      ...
      submit(id) {
        this.$axios.get('/condolence/selectionLeader', {
          params: {
            id: id
          }
        })
          .then((response) => {
            console.log(response);
            if (response.data == 0) {
              swal("非法操作", "此困难党员的慰问领导不可为空", "error");
            } else {
              this.$swal({
                title: "你确定？",
                text: "你将此困难党员领导入户慰问信息操作完成",
                showCancelButton: true,
                confirmButtonColor: "#1ab394",
                confirmButtonText: '是的，完成',
                showLoaderOnConfirm: true
              }).then((result) => {
                if (result.value) {
                  this.$axios.get('/condolence/submit', {
                    params: {
                      id: id
                    }
                  })
                    .then((response) => {
                      ...
                      swal("完成", "此困难党员领导入户慰问信息已完成", "success");
                      this.pageHandler(1, this.selectname, 'tongguotijiao');
                    })
                    .catch(function (error) {
                      console.log(error);
...
<style scoped>
  ...
  .my-swal {
    z-index: 2147483600111111111111111111111111111111111111111111111111111111;
  }
</style>
